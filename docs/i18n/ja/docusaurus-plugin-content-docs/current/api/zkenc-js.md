...
zkenc-jsの完全なAPIリファレンス、ウィットネス暗号化のためのJavaScript/TypeScriptライブラリ。
...
ウィットネス暗号化を使用してメッセージを暗号化します。鍵生成とAES-256-GCM暗号化を組み合わせます。
...
- `message` - 暗号化するメッセージ（Uint8Array形式）

**戻り値：**

- `Promise<EncryptResult>` - 結合された暗号文と暗号化鍵
...
- 初回呼び出し：~50-100ms（WASM初期化）
- 以降の呼び出し：~30-50ms

### `decrypt()`

ウィットネス復号化を使用してメッセージを復号化します。鍵を復元し、メッセージを復号化します。
...
- `circuitFiles` - 回路ファイル（R1CSとWASM）
- `ciphertext` - `encrypt()`からの結合された暗号文
- `inputs` - JSONオブジェクトとしての完全な入力（公開 + プライベートシグナル）
...
- 初回呼び出し：~150-200ms（WASM初期化 + ウィットネス計算）
- 以降の呼び出し：~100-150ms

## 低レベルAPI

低レベルAPIは、ウィットネス暗号化プロセスのきめ細かい制御を提供します。研究やカスタム暗号化スキームに使用します。

### `encap()`

ウィットネス暗号化を使用して暗号化鍵を生成（カプセル化）。
...
- `Promise<EncapResult>` - ウィットネス暗号文（1576バイト）と鍵（32バイト）
...
- `inputs` - JSONオブジェクトとしての完全な入力（回路を満たす必要がある）

**戻り値：**

- `Promise<Uint8Array>` - 復元された暗号化鍵（32バイト）

**例外をスロー：**

- `Error` - ウィットネスが回路制約を満たさない場合
...
公開としてマークされているシグナルまたは制約の一部だがウィットネスの一部ではないシグナルのみを含めます：
...
関数への初回呼び出しはWASMモジュールを初期化します（~20-50ms）。以降の呼び出しはより高速です。

### 回路の複雑さ

パフォーマンスは回路のサイズによって変わります：
