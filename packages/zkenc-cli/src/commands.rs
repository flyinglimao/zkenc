...
pub fn encap_command(
    ...
    let r1cs = R1csFile::from_file(circuit_path)
        .context("Failed to load R1CS circuit")?;
    ...
    let input_json = fs::read_to_string(input_path)
        .context("Failed to read input JSON file")?;
    ...
    println!("\n🔐 Running Encap...");
    let mut circuit = CircomCircuit::from_r1cs(&circuit_path)
    ...
    fs::write(ciphertext_path, &ciphertext_bytes)
        .context("Failed to write ciphertext file")?;
...
pub fn decap_command(
    ...
    let witness_file = WitnessFile::from_file(witness_path)
        .context("Failed to load witness file")?;
    
    println!("   - Witness elements: {}", witness_file.n_witness);
    
    // Convert witness to field elements
    let witness = witness_file.to_field_elements::<Fr>()
...
pub fn encrypt_command(
    ...
    let key = Key::deserialize_compressed(&key_bytes[..])
    ...
    println!("\n🔒 Encrypting...");
    let ciphertext = crypto::encrypt_gcm(key.as_bytes(), &plaintext)
...
pub fn decrypt_command(
    ...
    println!("\n🔓 Decrypting...");
    let plaintext = crypto::decrypt_gcm(key.as_bytes(), &ciphertext)
