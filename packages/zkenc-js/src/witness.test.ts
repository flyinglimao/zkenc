...
import { calculateWitness, calculateWitnessArray, getCircuitInfo } from './witness.js';
...
describe('Witness Calculator', () => {
  ...
  const wasmBuffer = new Uint8Array(readFileSync(join(fixtures, 'sudoku.wasm')));
  const inputData = JSON.parse(readFileSync(join(fixtures, 'sudoku_basic.json'), 'utf-8'));
